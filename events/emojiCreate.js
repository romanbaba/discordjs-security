...
module.exports = async(client, emoji) => {
    ...
    if(kanalKoruması) {
        ...
        const userPoint = db.fetch(`userPointEmoji_${emoji.guild.id}${executor.id}`) || 0
        ...
        if (executor) {
            ...
            if(userPoint >= l) {
                ...
                db.delete(`userPointEmoji_${emoji.guild.id}${executor.id}`)

            } else {

                const embed = new EmbedBuilder()
                .setColor("#FEE75C")
                .setDescription(`:warning: | **${executor.tag}** bir emoji oluşturdu, devam ederse onu banlayacağım! (${l}/**${userPoint}**)`)
                .setFooter({ text: `${executor.tag} tarafından oluşturuldu.`, iconURL: executor.displayAvatarURL({ dynamic: true }) })

                log.send({ embeds: [embed] })
                db.add(`userPointEmoji_${emoji.guild.id}${executor.id}`, 1)
            }
