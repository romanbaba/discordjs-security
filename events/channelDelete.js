...
module.exports = async(client, channel) => {
    ...
    if(kanalKoruması) {
        ...
        const userPoint = db.fetch(`userPoint_${channel.guild.id}${executor.id}`) || 0
        ...
        if (executor) {
            ...
            if(userPoint >= l) {
                ...
                db.delete(`userPoint_${channel.guild.id}${executor.id}`)

            } else {

                const embed = new EmbedBuilder()
                .setColor("#FEE75C")
                .setDescription(`:warning: | **${executor.tag}** bir kanal sildi, devam ederse onu banlayacağım! (${l}/**${userPoint}**)`)
                .setFooter({ text: `${executor.tag} tarafından silindi.`, iconURL: executor.displayAvatarURL({ dynamic: true }) })

                log.send({ embeds: [embed] })
                db.add(`userPoint_${channel.guild.id}${executor.id}`, 1)
            }
